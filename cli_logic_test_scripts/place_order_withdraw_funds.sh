...
#Mark order that has been delivered as completed and without dispute
npx near call $NEARID mark_order_as_completed '{"order_id": "'$ORDERID'"}' --account-id efama_marketplace.testnet

#withdraw funds as seller after a week that the order got closed
npx near call $NEARID withdraw_payment_for_order '{"order_id": "'$ORDERID'"}' --account-id $NEARID
